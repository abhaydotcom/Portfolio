...
function MagneticBtn({ children, className, to, variant = "primary" }) {
  ...
  const base =
    "relative inline-flex items-center justify-center gap-2 px-7 py-3.5 rounded-full text-sm font-semibold tracking-wide overflow-hidden transition-all duration-300 cursor-pointer";

  const variantStyles = {
    primary:
      "text-[#0a0a0f] bg-[#e2ff5d] hover:bg-white shadow-[0_0_30px_rgba(226,255,93,0.35)] hover:shadow-[0_0_50px_rgba(226,255,93,0.5)]",
    outline:
      "text-white border border-white/20 hover:border-white/40 bg-white/5 hover:bg-white/10 backdrop-blur-sm",
  };
  ...
  return (
    <Link to={to} className={className}>
      <motion.div
        ...
        whileTap={{ scale: 0.96 }}
        className={`${base} ${variantStyles[variant]} w-full`}
      >
        {children}
      </motion.div>
    </Link>
...
export default function Hero() {
  const roles = ["a Developer", "a Designer", "a Creator"];
    ...
    >
      ...
      <style>{`
        ...
        @media (min-width: 640px) {
          .stat-line::before { height: 32px; margin-bottom: 10px; }
        }
      `}</style>
      ...
      <motion.div
        ...
        style={{
          ...
          borderRadius: "50%",
          background:
            "radial-gradient(circle, rgba(226,255,93,0.07) 0%, transparent 65%)",
        }}
      />
      ...
      <div className="relative z-10 px-5 sm:px-8 w-full max-w-5xl mx-auto flex flex-col items-center pt-24 sm:pt-28 md:pt-32 pb-24 sm:pb-28">
        ...
        <motion.div {...fadeUp(0.2)} className="mb-1 sm:mb-2">
          <span className="text-base sm:text-lg md:text-xl font-light text-white/30 tracking-widest">
            Hey, I'm
          </span>
        </motion.div>
        ...
        <motion.h1
          {...fadeUp(0.3)}
          className="hero-name text-[2.8rem] xs:text-5xl sm:text-6xl md:text-7xl lg:text-[7rem] xl:text-[8.5rem] leading-[1.05] mb-3 sm:mb-4 text-white"
        >
          Abhay Kumar
          <br />
          <span
        ...
        <motion.div
          {...fadeUp(0.45)}
          className="mb-7 sm:mb-10 h-8 sm:h-10 flex items-center justify-center flex-wrap gap-x-1"
        >
          <span className="text-sm sm:text-base md:text-lg font-light text-white/40 tracking-widest uppercase">
            I am&nbsp;
          </span>
          <span className="role-cursor text-sm sm:text-base md:text-lg font-semibold text-[#e2ff5d] tracking-wide">
            {displayText}
          </span>
        </motion.div>
        ...
        <motion.p
          {...fadeUp(0.55)}
          className="max-w-xs sm:max-w-sm md:max-w-lg text-sm sm:text-base leading-relaxed text-white/40 font-light mb-8 sm:mb-12 px-2 sm:px-0"
        >
          Crafting{" "}
          <em className="text-white/70 not-italic font-normal">clean</em>,{" "}
          <em className="text-white/70 not-italic font-normal">scalable</em> web experiences
          where thoughtful engineering meets intentional design.
        </motion.p>


        <motion.div
          {...fadeUp(0.65)}
          className="flex flex-col xs:flex-row gap-3 sm:gap-4 items-center w-full xs:w-auto"
        >
          <MagneticBtn to="/projects" variant="primary" className="w-full xs:w-auto">
            View Work <Arrow />
          </MagneticBtn>
          <MagneticBtn to="/contact" variant="outline" className="w-full xs:w-auto">
            Get in Touch
          </MagneticBtn>
        </motion.div>
        ...
        >
          {[
            ...
            <div key={label} className="stat-line text-center">
              ...
              <div className="text-[9px] sm:text-[10px] tracking-[0.15em] sm:tracking-[0.2em] uppercase text-white/30 mt-1 font-medium">
                {label}
              </div>
            </div>
          ))}
